...
class DomainConversion:
    ...
    Args:
        depth_surfaces (list[xtgeo.RegularSurface]): List of depth surfaces.
        time_surfaces (list[xtgeo.RegularSurface]): List of time surfaces.
        names (Optional[list[str]]): Optional list of names for the surfaces. If not
    ...
    Example:
        ...
        >>> input_cube = xtgeo.cube_from_file("input_cube.segy")
        >>> result_cube = dc.depth_convert_cube(input_cube)
    """
    ...
    def _check_fix_surfaces(self) -> None:
        ...
        # use the lowermost surface as template for appropriate conversion
        template_surf = self.depth_surfaces[-1]
    ...
    def _resample_check_surfaces_to_cube(
        ...
        actual cube. In addition, we need to check that, after resampling, that
        surf no 1 (time or depth, depending on time2depth) is vertically inside the
        incube and the last surface is not deeper than the same cube. This is to
        ensure that the domain conversion becomes correct
        ...
        # check that the first "x" surf (exclude MSL!) is inside cube
        if x_surfaces[1].values.min() < incube.zori:
            raise ValueError(
                "First time or depth surface is deeper than the input cube. "
                "This is not allowed."
            )

        # check that the last time surface is not deeper than the cube
        if x_surfaces[-1].values.max() > self.max_depth_for_cube(incube):
            raise ValueError(
                "Last input time or depth surface is deeper than the input cube. "
                f"This is not allowed. Surface max is {x_surfaces[-1].values.max()} "
                f"and cube max is {self.max_depth_for_cube(incube)}. "
                "Consider setting `maxdepth` in the domain conversion."
            )
    ...
    @staticmethod
    def _cubes_are_similar(cube1: xtgeo.Cube, cube2: xtgeo.Cube) -> bool:
        """Check if two cubes are similar in shape, while values can differ."""
        return (
            cube1.ncol == cube2.ncol
            and cube1.nrow == cube2.nrow
            and cube1.nlay == cube2.nlay
            and cube1.xinc == cube2.xinc
            and cube1.yinc == cube2.yinc
            and cube1.zinc == cube2.zinc
            and cube1.xori == cube2.xori
            and cube1.yori == cube2.yori
            and cube1.zori == cube2.zori
            and cube1.rotation == cube2.rotation
            and cube1.yflip == cube2.yflip
        )
    ...
    def _extend_incube_create_speed_cube_average(
        ...
        If a speed cube with same topology already exists, it is no need to redo
        the calculations.
        """
        ...
        _logger.debug(
            "Check if a %s cube similar to input cube already exists",
            result_description,
        )
        if isinstance(_speed, xtgeo.Cube) and self._cubes_are_similar(_incube, _speed):
            _logger.debug(
                "Speed (%s) cube for this input cube shape already exists",
                result_description,
            )
            return
        ...
        # resample surfaces to match the input cube
        x_surfaces, s_surfaces = self._resample_check_surfaces_to_cube(
            _incube, time2depth
        )
    ...
    def depth_convert_cube(
        ...
        Note:
            ...
            provided. If given, the proposed values may be adjusted for technical
            reasons.
